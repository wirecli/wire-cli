<?php namespace Wirecli\Helpers\ProcessDiagnostics;
...
class DiagnosePhp extends ProcessDiagnostics {
  ...
  public function GetDiagnostics() {
    ...
    $pattern = preg_match('#</td>#msi',
      $buffer) === 1 ? '#>Server API.*?</td><td.*?>(.*?)</td>#msi' : '#\nServer API.*?=>(.*?)\n#msi';
    $api = preg_match($pattern, $buffer, $matches) === 1 ? trim($matches[1]) : null;

    $pattern = preg_match('#</td>#msi',
      $buffer) === 1 ? '#>System.*?</td><td.*?>(.*?)</td>#msi' : '#\nSystem.*?=>(.*?)\n#msi';
    ...
    if ($xdebug) {
      ...
      if ($xd_nestingLevel > 0 && $xd_nestingLevel < 190) {
        ...
        $xd_action .= '<br />' . sprintf('A value lower then 200 can result in problems, read more about it in the %s',
          '<a href="https://processwire.com/talk/tags/forums/xdebug/" target="_blank">PW Forums</a>.');
      } else {
        $xd_status = ProcessDiagnostics::$ok;
        $xd_action = sprintf('(*) xdebug.max_nesting_level = %d', $xd_nestingLevel);
        $xd_action .= '<br />' . sprintf('Read more on PW with XDebug in the %s',
          '<a href="https://processwire.com/talk/tags/forums/xdebug/" target="_blank">PW Forums</a>.');
      }
